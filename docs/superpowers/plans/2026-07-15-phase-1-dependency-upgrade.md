...
- `experimental.ppr` and `experimental.newDevOverlay` are dropped from `next.config.ts` entirely. **Corrected (found by Task 11's review):** the original "zero `<Suspense>` usage" premise for dropping `ppr` was wrong — `app/(login)/sign-in/page.tsx` and `sign-up/page.tsx` both wrap `<Login>` in `<Suspense>` for `useSearchParams()`'s static-bailout requirement. The real, verified reason `ppr` must go is stronger: Next 16 throws a hard `HardDeprecatedConfigError` at config-load time for any truthy `experimental.ppr` ("has been merged into `cacheComponents`") — confirmed by reading `next`'s own shipped `config.js` and reproducing the crash directly. `newDevOverlay` was removed from Next's own config type before Next 16 shipped, confirmed absent from the installed package.
...
**Correction (found by the whole-phase review, fixed 2026-07-16):** the "confirmed harmless" verdict above was correct as far as it went, but too narrowly scoped — it only checked that the `--sidebar-*` tokens *this task added* resolved identically to the pre-existing block's sidebar tokens, which was true. What it didn't check is that the surrounding second `:root`/`.dark`/`@theme inline` block those tokens were added into (present since a pre-phase `eb2742f "shadcn/ui update"` commit, not introduced by this task) was itself winning the CSS cascade over the *original* `:root`/`.dark` block for every non-sidebar token they both define — both are plain `:root` selectors, so the later one in file order wins. Confirmed live via `getComputedStyle`: `--radius` was resolving to the second block's `0.6rem`, not the original's `0.5rem` (a `rounded-md` button computed to 7.6px instead of 6px), and the entire neutral/primary palette was resolving to the second block's zinc hue instead of the original's slate hue that the app was actually designed around. Fixed in `app/globals.css` by deduping to one coherent token set: original slate palette + 0.5rem radius as the values, kept in the newer block's variable format (values pre-wrapped in `hsl(...)`, `--sidebar-*` naming) since `components/ui/sidebar.tsx`'s regenerated "outline" variant references `var(--sidebar-border)`/`var(--sidebar-accent)` directly as complete colors and would have broken under the original's raw-HSL-triplet format. See the `app/globals.css` commit "Fix globals.css duplicate theme tokens silently overriding the app's palette" for the full before/after. `tsc`/`build`/test suite reconfirmed green after the fix.
...
- [ ] `lib/payments/stripe.ts`'s `apiVersion` literal matches the version the installed SDK actually pins (verified below to be `'2026-06-24.dahlia'` by extracting the `ApiVersion` constant directly from `stripe@22.3.1`'s shipped `cjs/apiVersion.js` — re-confirm this if a different `stripe` patch ends up installed). **Correction (found + independently confirmed during implementation):** this premise was wrong — the `new Stripe(key, {...})` constructor's config type (`StripeConfig` in `lib.d.ts`) types `apiVersion` as `LatestApiVersion` (`typeof ApiVersion`), a true string-literal type, NOT the loose `string` from the separate `UserProvidedConfig`/`Types.d.ts` (used for internal per-request-options plumbing, which legitimately needs to accept any historical dated version). So `tsc` DOES reject a wrong/stale literal here — still worth sourcing the correct value from the installed package directly (not guessing), but you don't need to treat a stale value as a silent risk.
...
- [ ] `next.config.ts`'s `experimental` block is gone entirely: `ppr` dropped — **not** because of "zero `<Suspense>` usage" (that premise was wrong; sign-in/sign-up do use `<Suspense>`), but because Next 16 throws a hard `HardDeprecatedConfigError` at config-load time for any truthy `experimental.ppr` ("has been merged into `cacheComponents`"), confirmed by reading the installed `next` package's own `config.js` and reproducing the crash directly; `newDevOverlay` dropped because Next.js removed that flag from its own config type before Next 16 shipped (confirmed via `vercel/next.js` PR #76356, "`[dev-overlay] remove the experiment config for overlay`" — the redesigned dev overlay is simply on by default now, no flag needed)
...
**STATUS: DONE — this is the last task in Phase 1, all 11 tasks now complete.** Real, verified Turbopack incompatibility found in this environment: Next 16 now defaults *both* `next dev` and `next build` to Turbopack (previously only `next dev --turbopack` was explicit), and Turbopack cannot resolve `next/package.json` from `app/` given this sandbox's shared pnpm store — reproduced independently by two reviewers even with `turbopack.root` set and after a full clean reinstall. Worked around with `--webpack` on `build`, the Playwright e2e webServer, and (fixed in a follow-up commit after code-quality review caught the gap) the `dev` script too. This workaround is scoped to this specific sandbox's shared-store layout; whoever deploys this for real should verify whether the actual CI/production environment needs it or whether Turbopack can be re-enabled there. The codemod also added inert ESLint scaffolding (`eslint.config.mjs`, `eslint`/`eslint-config-next` deps, no `lint` script wired up, ~28 pre-existing lint errors elsewhere in the repo if it were ever run) — kept as-is since it's official codemod output and harmless while unused; wiring up linting and fixing that debt is a reasonable future task, not part of this dependency upgrade.

**Manual click-through checklist — evidence record.** The implementer's own report documents this was genuinely performed with real interaction, not just render checks: signed in with the seeded user (`test@test.com`/`admin123`) and confirmed redirect to `/dashboard`; created a new account via sign-up and confirmed redirect; `/dashboard` and its `general`/`security`/`activity`/`team` sub-pages all rendered with real data and no console errors; `/dashboard/chatbot` (static iframe) and `/dashboard/streaming` (real chat component) both rendered correctly, with the message round-trip itself unverifiable (no real AI Tutor API credentials, consistent with every other task's documented limitation); `/dashboard/workflow` and `/dashboard/get-token` rendered and their action buttons failed gracefully with an in-page error message (not a JS crash) when clicked without real credentials. This record is captured here because a later whole-phase review found no evidence of it in the commit messages — recording it now so the plan doc, not just the session transcript, reflects that it happened.
