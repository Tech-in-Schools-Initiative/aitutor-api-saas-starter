...
10. **stripe ^17.6.0 → 22.3.1** (5 majors, billing-critical). Update the pinned `apiVersion: '2025-01-27.acacia'` literal in `lib/payments/stripe.ts` to whatever the new SDK expects (it will fail to compile otherwise). Review Checkout Session / webhook Event payload changes across the range. **No Stripe test-mode/CLI access is available for this pass** — verification is limited to `tsc --noEmit`, `npm run build`, code review of the Checkout Session/webhook Event payload diffs against Stripe's changelog, and the existing unit/e2e suite. The checkout+webhook round trip itself (`stripe listen --forward-to localhost:3000/api/stripe/webhook` in test mode) is explicitly flagged as **your manual verification step** before this goes anywhere near production.
11. **`ai` package: attempt the full v4→v7 upgrade (user's explicit call, overriding this document's original freeze recommendation).** Go one major at a time (4→5→6→7), not in a single jump: `ai/react`'s `useChat` moves to `@ai-sdk/react` (new dependency), drops built-in input-state management (`input`/`handleInputChange`/`handleSubmit` are gone — `StreamingChat.tsx` must manage its own input state), `message.content` becomes a `parts` array, and `isLoading` becomes a `status` enum. Rewrite `StreamingChat.tsx` for each hop and re-run the Playwright chatbot smoke test (from step 0) after every single hop, not just at the end.
...
**shadcn registry default-styling drift (found during Phase 1 Task 5's component regeneration, confirmed by independent review, not yet acted on):** regenerating `components/ui/*` onto the current registry changed several component defaults from what this repo's design previously relied on. Live-impact-today items: `Avatar` default size shrank `h-10 w-10` → `size-8` (32px), affecting the bare `<Avatar>` in `dashboard/settings.tsx`'s team-member list (no size override there); `CardTitle`/`CardDescription` changed semantic tag `h3`/`p` → `div` (affects the ~5 files using these — check whether any relied on heading semantics for accessibility/SEO); `SheetTitle` lost its `text-lg` class. Not-yet-exercised-anywhere-but-worth-knowing items (currently dead code paths, so no visible effect today, but will matter the moment any of these components gets a real call site): `TooltipContent` flipped from brand-colored (`bg-primary`/`text-primary-foreground`) to neutral (`bg-foreground`/`text-background`); `Skeleton` background changed `bg-primary/10`→`bg-accent`; `RadioGroupItem`'s unchecked ring changed `border-primary`→`border-input`; `DropdownMenuSeparator` changed `bg-muted`→`bg-border`; stock `Button` variants lost most of their `shadow`/`shadow-sm` depth. Decide per-component whether the new registry default is fine as-is or needs a call-site override to preserve the original look. Do a fresh sweep rather than trusting this list as exhaustive — it came from a partial review pass during an unrelated task, not a dedicated audit.
...
**Streaming left alone:** `/api/chat` + `StreamingChat.tsx`'s `useChat` (on whichever `ai` version Phase 1 landed on) are untouched here — streaming chat state is not a React Query concern. `/api/run` is a single blocking `fetch`; if AI Tutor API's `/run` endpoint supports SSE/chunked responses (unconfirmed), streaming it would be a bigger UX win than `useMutation` alone.
...
4. **Next.js target is 16.2.10 stable** (a major-version jump from the current canary), not an intermediate Next-15 stable point release.
5. **TypeScript stays on the newest 5.x line**, deferring the 7.0 `tsgo` rewrite.
6. **`experimental.ppr` is dropped in Phase 1** rather than carried forward or built out with real Suspense boundaries.
7. **`packages/ui`/`packages/db`/`packages/email` ship raw TypeScript via `transpilePackages`**, not built to `dist/` via tsup.
8. **Generic `@repo/*` package scope** (no real npm org).
9. **`packages/db` stays framework-agnostic**; `getUser()` stays a thin Next-coupled wrapper in `apps/web/lib`.
10. **Team-invitation email is built alongside password-reset in Phase 3** (the template + send function; wiring into the existing `inviteTeamMember` TODO), since the incremental cost is low and it directly closes an already-flagged loop.
11. **`PASSWORD_RESET_SECRET` is not introduced** — the reset token reuses `AUTH_SECRET`, isolated by a `purpose: 'password-reset'` claim.
12. **No rate-limiting is added to the forgot-password endpoint** in this pass — flagged as a real gap (no rate-limiting infra exists anywhere in the repo today) but scoped out to avoid stalling Phase 3 on infrastructure the rest of the app doesn't have either.
